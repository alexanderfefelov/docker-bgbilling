...
APP_HOME=.
JMX="-javaagent:${APP_HOME}/lib/ext/jmx2graphite-1.2.4-agent-min.jar=SERVICE_NAME=bgbilling-accounting;GRAPHITE_HOSTNAME=graphite.bgbilling.local"
CLASSPATH=$APP_HOME:$APP_HOME/lib/ext/bgcommon-boot.jar:$APP_HOME/lib/ext/*
COMMON_PARAMS="-Dnetworkaddress.cache.ttl=3600 -Djava.net.preferIPv4Stack=true -Dboot.info=1 -Dapp.name=BGInetAccounting -Djava.endorsed.dirs=${BGBILLING_SERVER_DIR}/lib/endorsed:${JAVA_HOME}/lib/endorsed"
LOG_PARAMS="${JMX} -Dlog.dir.path=log/ -Dlog4j.configuration=log4j-accounting.xml"
