    ...
    decode[LegalEntityList](json) match {
      case Right(data) =>
        for (le <- data.legalEntities) {
          ...
          ContractActions.updateParameterType1(cid = le.id, pid = config.getInt("CONTRACT_PARAMETER_NAME"), value = le.name)
          ...
          le.serviceAddressOption.map(x => ContractActions.updateAddressInfo(cid = le.id, pid = config.getInt("CONTRACT_PARAMETER_SERVICE_ADDRESS"), hid = x.houseId, pod = x.entranceOption.getOrElse(0), floor = x.floorOption.getOrElse(0), flat = x.doorOption.getOrElse(""), room = x.roomOption.getOrElse("")))
          le.legalAddressOption.map(x => ContractActions.updateAddressInfo(cid = le.id, pid = config.getInt("CONTRACT_PARAMETER_LEGAL_ADDRESS"), hid = x.houseId, pod = x.entranceOption.getOrElse(0), floor = x.floorOption.getOrElse(0), flat = x.doorOption.getOrElse(""), room = x.roomOption.getOrElse("")))
          le.billingAddressOption.map(x => ContractActions.updateAddressInfo(cid = le.id, pid = config.getInt("CONTRACT_PARAMETER_BILLING_ADDRESS"), hid = x.houseId, pod = x.entranceOption.getOrElse(0), floor = x.floorOption.getOrElse(0), flat = x.doorOption.getOrElse(""), room = x.roomOption.getOrElse("")))
          ...
          le.codesOption.map { codes =>
            ...
            codes.okatoOption.map(x => ContractActions.updateParameterType1(cid = le.id, pid = config.getInt("CONTRACT_PARAMETER_OKATO"), value = x))
            codes.oktmoOption.map(x => ContractActions.updateParameterType1(cid = le.id, pid = config.getInt("CONTRACT_PARAMETER_OKTMO"), value = x))
