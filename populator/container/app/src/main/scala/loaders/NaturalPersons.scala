    ...
    decode[NaturalPersonList](json) match {
      case Right(data) =>
        for (np <- data.naturalPersons) {
          ...
          np.sexOption.map(x => ContractActions.updateListParameter(cid = np.id, pid = 39, value = x))
          ...
          np.idCardOption.map { idCard =>
            ...
            ContractActions.updateParameterType1(cid = np.id, pid = 11, value = idCard.number)
          ...
          np.legalAddressOption.map(x => ContractActions.updateAddressInfo(cid = np.id, pid = 3, hid = x.houseId, pod = x.entranceOption.getOrElse(0), floor = x.floorOption.getOrElse(0), flat = x.doorOption.getOrElse("")))
